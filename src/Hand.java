public class Hand {
  ...
  // create an array of cards and hand position, initialized to 12 because there
  // is no situation that the dealer or player
  // can have more than 12 cards without busting
  Card[] cards = new Card[12];
  // hand position is used to know the index at which a new card should be added;
  // hand position is increased whenever a card is added
  int handPosition = 0;
   ...
   * The dealer simply always chooses the higher value for an ace as long as it
   * doesn't bust in doing so.
   * This could be changed in such a way that the dealer takes into account a card
   * that it has not received yet,
   * and chooses a value for an ace accordingly. This could be considered
   * cheating. There are situations where if the
   * dealer chose the value of an ace to be 1, the sum of his hand would not be
   * 17, so he would hit again, but
   * if he chose it to be 11, the sum of his hand would be more than 21 and he
   * would stand. In this situation, because
